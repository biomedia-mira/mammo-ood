...
def collate_data_and_cast(
    ...
    if tissue_aware_mask and "tissue_masks" in samples_list[0][0]:
        collated_tissue_masks = torch.stack(
            [s[0]["tissue_masks"][i] for i in range(n_global_crops) for s in samples_list]
    ...
    if "y" in samples_list[0][0]:
        y0 = samples_list[0][0]["y"]
        if isinstance(y0, torch.Tensor):
            out["y"] = torch.stack([s[0]["y"] for s in samples_list])
        else:
            out["y"] = torch.tensor([s[0]["y"] for s in samples_list], dtype=torch.long)
    return out
