...
from architectures.dinov2.loss import DINOLoss, iBOTPatchLoss, KoLeoLoss
from architectures.dinov2.models import build_model_from_cfg
from architectures.dinov2.layers import DINOHead
from architectures.dinov2.utils.utils import has_batchnorms
from architectures.dinov2.utils.param_groups import get_params_groups_with_decay, fuse_params_groups
from architectures.dinov2.fsdp import get_fsdp_wrapper, ShardedGradScaler, get_fsdp_modules, reshard_fsdp_model

from architectures.dinov2.models.vision_transformer import BlockChunk
...
class SSLMetaArch(nn.Module):
    ...
    def forward_backward(self, images, teacher_temp, do_backward=True):
        n_global_crops = 2
        ...
        n_masked_patches_tensor = images["n_masked_patches"].to(device, non_blocking=True)
        ...
        with torch.no_grad():
            for k in self.student.keys():
                ...
                if student_fsdp_modules and teacher_fsdp_modules:
                    ...
                    student_param_list += [p for p in self.student[k].parameters() if p.requires_grad]
