...
from architectures.dinov2.configs import dinov2_default_config
from architectures.dinov2.data.collate import collate_data_and_cast
from architectures.dinov2.data.masking import MaskingGenerator
from architectures.dinov2.train.ssl_meta_arch import SSLMetaArch
from architectures.dinov2.utils.utils import CosineScheduler
...
from common.online_probe import MammoDensityProbeDataset, OnlineProbe
...
from methods.dinov2_augmentations import MammoDataAugmentationDINOv2, MammoValAugmentationDINOv2


mmap_file_path = str(EMBED_MEMMAP_512X384)
image_size = (512, 384)
local_image_size = (224, 168)


def _try_strip_prefixed_backbone(state_dict: dict[str, torch.Tensor], prefix: str) -> dict[str, torch.Tensor]:
    return {
...
def _save_backbone_exports(state_dict: dict[str, torch.Tensor], output_dir: str, *, suffix: str = "") -> None:
    student_backbone = _try_strip_prefixed_backbone(state_dict, "student.backbone.")
    teacher_backbone = _try_strip_prefixed_backbone(state_dict, "teacher.backbone.")
    if not student_backbone:
        raise ValueError("Could not find any weights for prefix 'student.backbone.'")
    if not teacher_backbone:
        raise ValueError("Could not find any weights for prefix 'teacher.backbone.'")
    torch.save(student_backbone, os.path.join(output_dir, f"dinov2_student_backbone{suffix}.pth"))
    torch.save(teacher_backbone, os.path.join(output_dir, f"dinov2_teacher_backbone{suffix}.pth"))


def _normalize_dinov2_arch_name(model_name: str) -> str:
    aliases = {
        'dino_vit_small': 'vit_small',
        'dino_vit_base': 'vit_base',
        'dino_vit_large': 'vit_large',
    }
...
def _default_channel_stats(input_channels: int, normalization: str = 'imagenet') -> tuple[list[float], list[float]]:
    ...
    # ImageNet stats match the original DINOv2 pretrain distribution.
...
def _resolve_official_epoch_length(datamodule: pl.LightningDataModule, *, world_size: int = 1) -> int:
    ...
    try:
        ...
        raise RuntimeError(
            "Could not determine DINOv2 OFFICIAL_EPOCH_LENGTH from the train dataloader. "
            "The train loader must implement __len__ so the optimizer schedule can be built correctly."
        ) from exc


class MammoDatasetDINOv2(BaseMammoMMapDataset):
...
class EMBEDMammoDataModuleDINOv2(pl.LightningDataModule):
    def __init__(
        self,
        *,
        data_dir: str,
        ...
        mean: Sequence[float],
        std: Sequence[float],
        online_probe_enabled: bool = True,
        ...
        self.input_channels = int(input_channels)
        self.global_crops_scale = tuple(float(v) for v in global_crops_scale)
        self.local_crops_scale = tuple(float(v) for v in local_crops_scale)
        self.local_crops_number = int(local_crops_number)
        self.local_crops_size = tuple(int(v) for v in local_crops_size)
        self.patch_size = int(patch_size)
        self.ibot_mask_ratio_min_max = tuple(float(v) for v in ibot_mask_ratio_min_max)
    ...
    def setup(self, stage=None):
        train_transform = MammoDataAugmentationDINOv2(
        ...
        val_transform = MammoValAugmentationDINOv2(
            global_crops_size=self.image_size,
            local_crops_size=self.local_crops_size,
            local_crops_number=self.local_crops_number,
            input_channels=self.input_channels,
        ...
        self.dataset_train = MammoDatasetDINOv2(
            self.train_data,
            transform=train_transform,
            mmap_path=self.mmap_path,
            input_channels=self.input_channels,
        )
        self.dataset_val = MammoDatasetDINOv2(
            self.val_data,
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            ...
            print('samples (val):   ', len(self.dataset_val))
...
def _build_dinov2_cfg(hparams, output_dir: str):
    ...
    cfg.train.centering = hparams.centering

    cfg.compute_precision.grad_scaler = False

    cfg.student.arch = _normalize_dinov2_arch_name(hparams.model)
    cfg.student.patch_size = hparams.patch_size
    cfg.student.num_register_tokens = hparams.num_register_tokens
    if hparams.num_register_tokens > 0:
        cfg.student.interpolate_antialias = True
        cfg.student.interpolate_offset = 0.0
    ...
    cfg.crops.rgb_mean, cfg.crops.rgb_std = _default_channel_stats(
    ...
    cfg.ibot.separate_head = hparams.ibot_separate_head
    ...
    wd_start = hparams.wd_start if hparams.wd_start is not None else hparams.weight_decay
    cfg.optim.epochs = hparams.epochs
    cfg.optim.lr = hparams.learning_rate
    cfg.optim.base_lr = hparams.learning_rate
    cfg.optim.weight_decay = wd_start
    cfg.optim.weight_decay_end = hparams.wd_end
...
class DINOv2Lightning(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        num_classes: int = 4,
        online_probe_enabled: bool = True,
        online_probe_learning_rate: float = 1e-4,
        online_probe_weight_decay: float = 1e-4,
    ...
    def _build_model(self):
        ...
        self.ssl_model.need_to_synchronize_fsdp_streams = False

        expected_in_chans = int(self.cfg.student.get('in_chans', 3))
        ...
        if student_in_chans != expected_in_chans or teacher_in_chans != expected_in_chans:
            raise RuntimeError(
                "DINOv2 model/data channel mismatch during build: "
    ...
    def _sync_teacher_from_student(self) -> None:
        for key in self.ssl_model.student.keys():
            self.ssl_model.teacher[key].load_state_dict(self.ssl_model.student[key].state_dict(), strict=True)
        self.print('[DINOv2] Synced teacher modules from student.')

    @staticmethod
    def _interpolate_pos_embed(pos_embed: torch.Tensor, model) -> torch.Tensor:
        cls_pos = pos_embed[:, :1, :]
        patch_pos = pos_embed[:, 1:, :]
        num_patches_ckpt = patch_pos.shape[1]
        dim = patch_pos.shape[-1]

        side_ckpt = int(math.sqrt(num_patches_ckpt))
        if side_ckpt * side_ckpt != num_patches_ckpt:
            side_ckpt_h = side_ckpt_w = side_ckpt
            for h in range(side_ckpt, 0, -1):
                if num_patches_ckpt % h == 0:
                    side_ckpt_h, side_ckpt_w = h, num_patches_ckpt // h
                    break
        else:
            side_ckpt_h = side_ckpt_w = side_ckpt

        patch_embed = model.patch_embed
        h_model = patch_embed.img_size[0] // patch_embed.patch_size[0]
        w_model = patch_embed.img_size[1] // patch_embed.patch_size[1]

        patch_pos = patch_pos.reshape(1, side_ckpt_h, side_ckpt_w, dim).permute(0, 3, 1, 2).float()
    ...
    def _extract_backbone_state_dict(self, state_dict: dict[str, Any]) -> dict[str, torch.Tensor]:
        ...
        for prefix in (
            ...
            extracted = _try_strip_prefixed_backbone(state_dict, prefix)
            if extracted:
                return extracted
        return state_dict

    def _load_pretrained_backbone(self, ckpt_path: str) -> None:
        self.print(f'[DINOv2] Loading pretrained backbone from {ckpt_path}')
        state_dict = torch.load(ckpt_path, map_location='cpu')
        state_dict = self._extract_backbone_state_dict(state_dict)

        student_backbone = self.ssl_model.student.backbone
        student_sd = student_backbone.state_dict()
        load_dict = {}
        skipped = []
        for key, value in state_dict.items():
            if key == 'pos_embed' and key in student_sd and value.shape != student_sd[key].shape:
                self.print(f'[DINOv2] Interpolating pos_embed: {value.shape} -> {student_sd[key].shape}')
                value = self._interpolate_pos_embed(value, student_backbone)
            if key in student_sd:
                if value.shape == student_sd[key].shape:
                    load_dict[key] = value
                else:
                    skipped.append(f'{key} (shape {tuple(value.shape)} vs {tuple(student_sd[key].shape)})')
            else:
                skipped.append(f'{key} (not in model)')

        missing, unexpected = student_backbone.load_state_dict(load_dict, strict=False)
        self.print(f'[DINOv2] Loaded {len(load_dict)} params into student backbone')
        self.print(f'[DINOv2] Skipped from ckpt: {skipped}')
        self.print(f'[DINOv2] Missing in student (kept random): {list(missing)}')
        if unexpected:
            self.print(f'[DINOv2] Unexpected checkpoint keys: {list(unexpected)}')

        teacher_backbone = self.ssl_model.teacher.backbone
        teacher_backbone.load_state_dict(student_backbone.state_dict(), strict=True)
        self.print('[DINOv2] Pretrained backbone loaded into student and teacher.')
    ...
    def on_fit_start(self):
        ...
        if official_epoch_length <= 1:
            ...
            self.print(
                f"[DINOv2][Debug] OFFICIAL_EPOCH_LENGTH fallback from trainer.num_training_batches -> {official_epoch_length}"
            )
    ...
    def _build_schedules(self):
        ...
        self.last_layer_lr_schedule = CosineScheduler(
            base_value=self.cfg.optim.lr,
            final_value=self.cfg.optim.min_lr,
            total_iters=total_iters,
            warmup_iters=warmup_epochs * official_epoch_length,
        ...
        self.last_layer_lr_schedule.schedule[:freeze_iters] = 0
    ...
    def training_step(self, batch, batch_idx):
        ...
        optimizer.step()
        self.ssl_model.update_teacher(mom_val)

        batch_size = self.cfg.train.batch_size_per_gpu
        self.log('train_total_loss', total_loss.detach(), on_step=False, on_epoch=True, prog_bar=True, sync_dist=True, batch_size=batch_size)
        ...
        self.log('ema_tau', mom_val, on_step=True, on_epoch=False, sync_dist=True, batch_size=batch_size)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        it = min(
            ...
            max(int(self.cfg.train.OFFICIAL_EPOCH_LENGTH) * max(int(self.cfg.optim.epochs), 1) - 1, 0),
        )
        teacher_temp_val = self._schedule_value(self.teacher_temp_schedule, it, self.cfg.teacher.teacher_temp)

        with torch.no_grad():
            val_loss, metrics_dict = self.ssl_model.forward_backward(
                ssl_batch,
                teacher_temp=teacher_temp_val,
                do_backward=False,
            )
    ...
    def forward(self, x):
        return self.ssl_model.teacher.backbone.forward_features(x)['x_norm_clstoken']
...
def main(hparams):
    ...
    cfg = _build_dinov2_cfg(hparams, output_dir)
    ...
    data = EMBEDMammoDataModuleDINOv2(
        data_dir=str(EMBED_PNG_1024X768_DIR),
...
def build_parser():
    ...
    parser.add_argument('--wd_start', type=float, default=None)
    parser.add_argument('--wd_end', type=float, default=0.4)
    ...
    parser.add_argument(
        '--model',
        type=str,
        default='dino_vit_base',
        choices=['dino_vit_small', 'dino_vit_base', 'dino_vit_large', 'vit_small', 'vit_base', 'vit_large', 'vit_giant2'],
    )
    ...
    parser.add_argument('--local_crops_scale', type=float, nargs=2, default=[0.15, 0.40])
    ...
    parser.add_argument('--centering', type=str, default='centering', choices=['centering', 'sinkhorn_knopp'])
    parser.add_argument('--koleo_loss_weight', type=float, default=0.05)
    parser.add_argument('--ibot_loss_weight', type=float, default=1.0)
    parser.add_argument('--ibot_separate_head', type=parse_bool, default=False)
    ...
    parser.add_argument('--use_lr_scheduler', type=parse_bool, default=True)
    ...
    parser.add_argument('--output_name', type=str, default='dinov2_embed_run')
    ...
    parser.add_argument('--online_probe_enabled', type=parse_bool, default=True)
