...
from architectures.dinov2.data.transforms import GaussianBlur
from methods.dino_common import NoEmptyCrop, TissueAwareCrop


class MammoDataAugmentationDINOv2:
    def __init__(
        self,
        *,
        ...
        flip_p = 0.5 if self.horizontal_flips else 0.0
        crop_wrapper = TissueAwareCrop if self.tissue_aware_crop else NoEmptyCrop
        self.geometric_augmentation_global = crop_wrapper(tf.Compose([
            ...
            tf.RandomHorizontalFlip(p=flip_p),
        ]))
        self.geometric_augmentation_local = crop_wrapper(tf.Compose([
            tf.RandomResizedCrop(
        ...
        if random_color_jitter > 0 and color_jitter_prob > 0:
            color_jitter = tf.RandomApply([
                tf.ColorJitter(
                    brightness=float(random_color_jitter),
                    contrast=float(random_color_jitter),
                )
            ], p=float(color_jitter_prob))
        else:
            color_jitter = torch.nn.Identity()

        normalize = tf.Normalize(mean=self.mean, std=self.std)
        self.global_transfo1 = tf.Compose([color_jitter, GaussianBlur(p=1.0), normalize])
        self.global_transfo2 = tf.Compose([color_jitter, GaussianBlur(p=0.1), normalize])
        self.local_transfo = tf.Compose([color_jitter, GaussianBlur(p=0.5), normalize])
...
class MammoValAugmentationDINOv2:
    def __init__(
        self,
        *,
        global_crops_size: Sequence[int],
        ...
        self.global_base_transform = tf.Compose([
            tf.Resize(self.global_crops_size, interpolation=tf.InterpolationMode.BICUBIC, antialias=True),
            tf.CenterCrop(self.global_crops_size),
        ])
        self.local_base_transform = tf.Compose([
            tf.Resize(self.local_crops_size, interpolation=tf.InterpolationMode.BICUBIC, antialias=True),
            tf.CenterCrop(self.local_crops_size),
        ])
        self.normalize = tf.Normalize(mean=mean, std=std)
